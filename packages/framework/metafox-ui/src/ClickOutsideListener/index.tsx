 ...
 * API:
 *
 * - [ClickOutsideListener API](https://material-ui.com/api/click-away-listener/)
 */
function ClickOutsideListener(props: ClickOutsideListenerProps): JSX.Element {
  const {
    children,
  ...
  const handleClickAway = useEventCallback((event: MouseEvent | TouchEvent) => {
    ...
    if (event.composedPath) {
      insideDOM =
        ...
        (excludeRef?.current &&
          event.composedPath().indexOf(excludeRef.current) > -1);
    } else {
      insideDOM =
  ...
  React.useEffect(() => {
    if (touchEvent !== false) {
      ...
      doc.addEventListener(mappedTouchEvent, handleClickAway as any);
      doc.addEventListener('touchmove', handleTouchMove);

      return () => {
        doc.removeEventListener(mappedTouchEvent, handleClickAway as any);
  ...
  React.useEffect(() => {
    if (mouseEvent !== false) {
      ...
      doc.addEventListener(mappedMouseEvent, handleClickAway as any);

      return () => {
        doc.removeEventListener(mappedMouseEvent, handleClickAway as any);
      };
    }
