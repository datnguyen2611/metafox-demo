...
export function* handleActionFeedback(
  ...
  if (message) {
    ...
    if (status === 'success') yield toastBackend.success(message);

    if (status === 'warning') yield toastBackend.warning(message);

    if (status === 'error') yield toastBackend.error(message);

    if (status === 'info') yield toastBackend.info(message);
  }
