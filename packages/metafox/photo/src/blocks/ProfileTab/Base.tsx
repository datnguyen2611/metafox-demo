...
export default function ProfileTabPhoto(props: UIBlockViewProps) {
  ...
  const labelPhotoOfUser = authorUser.link === userLogin.link 
  ? 'tab_photos_of_you' 
  : i18n.formatMessage({ id: 'tab_photo_of_user' }, { user: authorUser.full_name });

  const labelYourPhoto = authorUser.link === userLogin.link 
  ? 'tab_your_photos' 
  : i18n.formatMessage({ id: 'tab_user_s_photos' }, { user: authorUser.full_name });
