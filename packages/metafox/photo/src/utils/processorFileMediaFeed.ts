import { isVideoType, isPhotoType } from '@metafox/utils';
import { uniqueId } from 'lodash';

type FileError = File & { maxSize: number };
type FileItem = { uid: string; source: string; file: File };

function validateFileInputType(type, accept) {
  // accept all file
...
export const checkFileMediaFeed = ({
  ...
  for (let i = 0; i < files.length; ++i) {
    ...
    if (
      !validateFileInputType(fileItem?.type, accept) ||
