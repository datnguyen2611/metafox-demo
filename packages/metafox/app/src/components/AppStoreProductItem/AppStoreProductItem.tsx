...
const Title = styled(ItemTitle, { name: 'Title' })(({ theme }) => ({
  display: 'block'
}));
const LinkAuthor = styled(RouteLink, { name: 'LinkAuthor' })(({ theme }) => ({
  display: 'inline-flex'
}));
const MainContent = styled(Box, { name: 'Title' })(({ theme }) => ({
  display: 'flex',
  alignItems: 'flex-start',
  justifyContent: 'space-between'
}));

const FooterContent = styled(Box, { name: 'FooterContent' })(({ theme }) => ({
...
const ListItem = styled(ItemView, { name: 'ListItem' })(({ theme }) => ({
  color: theme.palette.text.primary
}));

const OverlayLink = styled(RouteLink, { name: 'OverlayLink' })(({ theme }) => ({
  position: 'absolute',
  left: 0,
  right: 0,
  bottom: 0,
